...
EddyFlow processes raw eddy covariance (EC) data to compute biospheric/atmospheric fluxes of CO2, H2O, CH4, other trace gases, and energy. It is based on the [ECO2S](http://gaia.agraria.unitus.it/eco2s) package, which was released as free software by the consortium participating in the IMECC-EU European project. EddyFlow is customized to efficiently process eddy covariance data logged to LI-COR gas analyzers. It also supports other raw file types, including data stored as ASCII tables, binary files (including the TOB1 format), and SLT (EddySoft and EdiSol) formats.

In 2010, LI-COR Biosciences selected this software platform as the foundation for EddyPro, its eddy covariance data processing and analysis package. EddyFlow is an open-source fork of EddyPro, developed and maintained by ETH Zurich, that continues this effort with new processing methods, a rebuilt multi-gas architecture, and ongoing bug fixes. EddyFlow is designed to provide easy, accurate EC flux computations.

EddyFlow is released as free, open source software (GNU General Public License v3) and is developed and maintained on GitHub.

**Suggested Software Reference:** EddyFlow (n.d.). ETH Zurich [Computer software]. Available at the project's GitHub repository.
...
How does EddyFlow make this possible? Inspired by the ECO2S philosophy, LI-COR designed an innovative raw file format (called the .ghg format, denoted by a .ghg file extension) that embeds all relevant meta-information inside the data file. Because each raw file includes all the information needed to be properly interpreted and processed, each raw file can be handled independently from the others. LI-COR eddy covariance data files (.ghg extension) are automatically created by properly configured LI-COR data logging software. EddyFlow also allows you to calculate corrected fluxes starting from raw files in other formats, such as generic ASCII, Binary, TOB1, and SLT files, as well as by importing existing EddyPro or EddyUH projects. In these cases, meta-information must be provided before proceeding through the EddyFlow graphical interface.
...
EddyFlow is compatible with computers running Windows and macOS, and is also built for Linux. Installing the software is a simple matter of downloading it, and then launching the executable file that is stored on your computer. On Windows, the executable file is called eddyflow.exe. On macOS, it is EddyFlow.app (distributed as EddyFlow.dmg).
