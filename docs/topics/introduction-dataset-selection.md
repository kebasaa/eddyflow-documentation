...
!!! note
    ...
    An entire record (that is, all variables measured at a certain time instant, one line of raw data) is eliminated any time a flag variable does not comply with its quality criterion. See [Filtering raw data records for instrument diagnostics and custom flags](flags.md#top).
