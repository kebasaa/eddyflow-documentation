...
**Correction for instruments separation:** Applies an additional correction for the physical separation between the anemometer and the gas analyzer, on top of whichever low-pass method is selected above.

**Cospectral model:** Selects the analytical (co)spectral model used when fitting a reference cospectrum for the correction, for example a Kaimal-type model such as Moncrieff et al. (1997). See [Calculating spectra, cospectra and ogives](calculate-spectra-cospectra-and-ogives.md#top).

**Iterate the correction:** *(Added in engine v8.1.0.)* When checked, the spectral correction and the flux computation are repeated until the atmospheric stability the correction assumes and the stability the corrected flux produces agree, rather than being computed once from the uncorrected flux's stability. **Passes** sets the maximum number of iterations; **Stop below** sets a convergence tolerance below which the loop exits early. Leaving **Stop below** at *run every pass* always runs the full number of passes. This is off by default; when off, the correction is computed once, as in earlier versions.
